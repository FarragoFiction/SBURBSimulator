...
window.onload = () {
  ...
  if(!debugMode){
  	...
  	for(num i = 0; i<getRandomInt(10,30); i++){
...
void tick(){
	querySelector("#story").append(processScenes(curSessionGlobalVar.players,curSessionGlobalVar));
}



void reckoningTick(){
	querySelector("#story").append(processReckoning(curSessionGlobalVar.players,curSessionGlobalVar));
}
...
void printPlayers(){
	for(num i = 0; i<players.length; i++){
		querySelector("#story").append(players[i].title() + " in the " + players[i].land +"<br>");
	}
}
...
dynamic introduceReckoning(){
	String intro = " The reckoning has begun.  The Black King has defeated his Prospitian counterpart, initiating a meteor storm to destroy Skaia. ";
...
void conclusion(){
	...
	querySelector("#story").append(end);
...
void introducePlayers(){
	for(num i = 0; i<curSessionGlobalVar.players.length; i++){
		...
		String intro = "The " + p.htmlTitle() + " enters the game " + indexToWords(i) + ". ";
		if(i == 0){
		...
		for(num j = 0; j<p.relationships.length; j++){
		...
		querySelector("#story").append(processScenes(playersInMedium,curSessionGlobalVar));
	}
}
...
dynamic isThereMeetup(playersInMedium){
	String meetup = "";
