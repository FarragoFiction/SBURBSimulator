function VoidyStuff(session){
	...
	this.renderContent = function(div){
		...
		this.player.increasePower();
		if(this.enablingPlayer.aspect == "Void") this.player.corruptionLevelOther += getRandomInt(1,2); //void isn't a safe place to be.
	...
	this.chooseShenanigans = function(div){
		...
		if(this.player.godDestiny && !this.player.godTier && Math.seededRandom()>0.8 && this.player.land != null){
			...
			this.endingPhrase(classDiv, newDiv);
			return;
		}else if(this.player.triggerLevel > 5 && !this.player.murderMode && Math.seededRandom() > 0.98){
