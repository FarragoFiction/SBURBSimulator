...
window.onload = function() {
	$(this).scrollTop(0);
	loadNavbar();
	debug("<h1>Extremely fucking alpha</h1>, beware. Even if you find a bug, odds are the code will be completely different a day later. <Br><br>Current status: can override claspects.  Switching over to overhauling claspects. ");
	debug("Redo this page entirely. No longer do sessions in same page. This page is ONLY for customization. Instead, use afterlife URL code to transfer customized player data to index2.html. Make sure to load customized data before easter eggs and during yellow yard/scratch bullshit. Have regular shareable URL and custom character shareable URL (shareable URL will be a goddamned paragraph till i figure out a decent compression scheme). Clicking 'replay' from a session that already has custom characters has to transfer those back to this page.")
