...
class ExileQueen extends Scene {
	...
	bool trigger(playerList){
		...
		return (this.session.npcHandler.queen.getStat("power")<10 && !this.session.npcHandler.queen.exiled );
	}
	dynamic content(){
		session.logger.info("trying to exile queen: " + this.session.session_id.toString());
		String ret = "";
		if(this.session.npcHandler.queen.getStat("currentHP") > 0 && this.session.npcHandler.queen.crowned != null){
		...
		}else if(this.session.npcHandler.queen.crowned !=null  && this.session.npcHandler.queen.getStat("currentHP") <= 0){
			...
			this.session.destroyBlackRing();
		}else if(this.session.npcHandler.queen.getStat("currentHP") > 0){
		...
		this.session.npcHandler.queen.setStat("currentHP", -999990); //effectively dead.
