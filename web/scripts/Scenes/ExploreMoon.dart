...
class ExploreMoon extends Scene {
	...
	void checkPlayer(Player player) {
		...
		String getProspitBullshit(){
			...
			if(this.session.timeTillReckoning < 3 && getAveragePower(living)>this.session.npcHandler.king.getStat(Stats.CURRENT_HEALTH)*2) vision = "Wow, everybody sure looks happy in that cloud. Maybe things will turn out after all?";
			if(this.session.timeTillReckoning < 3 && this.session.npcHandler.king.getStat(Stats.POWER)> getAverageHP(living) * 2) vision = "Um...Huh. That's. That's a lot of dead bodies. What's...what's going on in that cloud there?";
		...
		String getDerseBullshit(){
			...
			if(this.session.timeTillReckoning < 3 && this.session.npcHandler.king.getStat(Stats.POWER)> getAverageHP(living) * 2) whisper = "Oh god, did the Horrorterrors get LOUDER!?";
