...
abstract class Scene {
  ...
  static void createScenesForDeadSession(Session session) {
    session.scenes = [new GetWasted(session),new DeadQuests(session),new DeadMeta(session),new StartDemocracy(session), new JackBeginScheming(session), new GiveJackBullshitWeapon(session), new JackPromotion(session), new JackRampage(session)];
    //relationship drama has a high priority because it can distract a session from actually making progress. happened to universe a trolls.
